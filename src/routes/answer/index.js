...
router.get(
	"/vote/id=:answerId",
	passport.authenticate(["jwt"], { session: false }),
	api4Answer.isVotedAnswer.handler
);
// router.get(
// 	"/vote/id=:answerId",
// 	passport.authenticate(["jwt"], { session: false }),
// 	validate("body", api4Answer.isVotedAnswer.validation),
// 	api4Answer.isVotedAnswer.handler
// );
router.get(
	"/id=:answerId",
	passport.authenticate(["jwt"], { session: false }),
	api4Answer.getAnswerbyId.handler
);
...
router.put(
	"/upVote/id=:answerId&downVote=:downVote&upVote=:upVote",
	passport.authenticate(["jwt"], { session: false }),
	api4Answer.upVoteAnswer.handler
);
router.put(
	"/downVote/id=:answerId&downVote=:downVote&upVote=:upVote",
	passport.authenticate(["jwt"], { session: false }),
	api4Answer.downVoteAnswer.handler
);
